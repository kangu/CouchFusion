...
## Initial Prompt
After git clone commands, the repositories (currently for the layers and the app created with new) are to be cleared of their git histories and reinitialized as a fresh git repository.

## Implementation Summary
Implementation Summary: Added post-clone git cleanup that strips upstream history and reinitializes repositories across init, new, and create_layer workflows.
...
- Executing `couchfusion new my-app` yields a new app under `apps/my-app` whose `.git` directory starts clean while subsequent environment tweaks appear as new changes.
