...
## Initial Prompt
When running the create_app command, first check to see if the cli is ran inside a valid couchfusion workspace, and if not, exit immediately. Allow passing the app_name parameter as the second parameter after create_app, so if someone calls "couchfusion create_app", it asks for the name, if the call is "couchfusion create_app testapp", then use "testapp" as the name parameter. Improve the TUI interface with nice colors and a nice feel based on Bubble Tea and Lip Gloss
...
- `cli-init/internal/workspace/create_app_tui.go:1` implements the multi-step wizard (name entry, module selection, auth credential collection, progress spinner, completion view) with Bubble Tea and Lip Gloss styling.
...
- Running `couchfusion create_app testapp` pre-fills the name step in the TUI and guides the user through module selection (with arrow/space controls) and optional auth credential entry.
